        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              <span
                ...
                style={{ boxShadow: '0 0 10px var(--green)', animation: 'pulseDot 2s ease-in-out infinite' }}
                aria-hidden="true"
              />
            ...
            <h1
              ...
              style={{ fontSize: 'clamp(42px, 5.5vw, 72px)', animation: 'fadeUp 0.7s ease both 0.2s' }}
            >
              Hi, I&apos;m
              <br />
            ...
            >
              ...
              2+ years of turning complex problems into elegant digital solutions.
            </p>

            {/* CTAs */}
            <div className="flex flex-wrap gap-4" style={{ animation: 'fadeUp 0.7s ease both 0.55s' }}>
              <button onClick={() => scrollTo('#projects')} className="c-btn-primary">
                View My Work
                <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M5 12h14M12 5l7 7-7 7" />
                </svg>
              </button>
              <button onClick={() => scrollTo('#contact')} className="c-btn-secondary">
                Get in Touch
                <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
            ...
            >
              {[['2+', 'Years Exp.'], ['15+', 'Projects'], ['10+', 'Technologies']].map(([n, l]) => (
                <div key={l} role="listitem" className="flex flex-col gap-1">
                  <span
                    className="font-syne text-3xl font-black bg-gradient-to-r from-[var(--accent)] to-[var(--accent3)] bg-clip-text text-transparent"
                  >{n}</span>
                  <span className="text-xs text-[var(--text3)] tracking-wide">{l}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <div className="relative w-full max-w-[420px] aspect-square rounded-[28px] bg-[var(--surface)] border border-[var(--border2)] backdrop-blur-xl overflow-hidden flex items-center justify-center">
              {/* Gradient rim */}
              <div className="absolute inset-[-1px] rounded-[28px] -z-10"
                style={{ background: 'linear-gradient(135deg, rgba(108,99,255,0.27), transparent 50%, rgba(56,189,248,0.2))' }} />

              <div className="p-7 w-full font-mono text-[13px] leading-[1.85]">
                <div className="text-[var(--text3)] italic">{'// Rajesh Sarkar'}</div>
                <div className="text-[var(--text3)]"><span className="text-[#a78bfa]">const</span> <span className="text-[#38bdf8]">developer</span> = {'{'}</div>
                ...
                <div className="text-[var(--text3)]">&nbsp;&nbsp;stack: [</div>
                <div className="text-[var(--text3)]">&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-[#34d399]">&quot;React&quot;</span>, <span className="text-[#34d399]">&quot;Next.js&quot;</span>,</div>
                <div className="text-[var(--text3)]">&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-[#34d399]">&quot;Node.js&quot;</span>, <span className="text-[#34d399]">&quot;MongoDB&quot;</span></div>
                <div className="text-[var(--text3)]">&nbsp;&nbsp;],</div>
                <div className="text-[var(--text3)]">&nbsp;&nbsp;available: <span className="text-[#a78bfa]">true</span></div>
                <div className="text-[var(--text3)]">{'};'}</div>
                <br />
                <div className="text-[var(--text3)]"><span className="text-[#38bdf8]">console</span>.<span className="text-[#38bdf8]">log</span>(<span className="text-[#34d399]">&quot;Let&apos;s build!&quot;</span>);</div>
              </div>
            </div>

            {/* Floating tags */}
            {[
              { label: '⚛️ React.js',  style: { top: '12%', right: '-12%', animationDelay: '-2s' } },
              { label: '🟢 Node.js',   style: { bottom: '15%', left: '-12%', animationDelay: '-1s' } },
              { label: '🍃 MongoDB',   style: { top: '60%', right: '-16%', animationDelay: '-3s' } },
            ].map((t) => (
              <span
                key={t.label}
                ...
                style={{ ...t.style, boxShadow: 'var(--shadow)', animation: `floatTag 4s ease-in-out infinite ${t.style.animationDelay}` }}
              >
                {t.label}
              </span>
            ))}
